      ...
      <div className="container-editorial py-20 md:py-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-1">
            <Link
              href="/"
              className="font-display text-2xl text-ivory hover:text-gold transition-colors duration-300 block mb-4"
            >
              Yaw Tutu
            </Link>
            <p className="text-sm leading-relaxed text-ivory/60 mb-6 max-w-xs">
              Transformational education in tantra, conscious intimacy, and embodied connection — for curious, growth-oriented adults.
            </p>
            ...
            <ul className="space-y-3">
              {footerLinks.experiences.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-ivory/60 hover:text-ivory transition-colors duration-300 link-underline"
                  >
                    {link.label}
                  </Link>
                </li>
          ...
          <div>
            ...
            <p className="text-sm text-ivory/60 mb-4 leading-relaxed">
              Reflections on embodiment, connection, and conscious living — delivered occasionally.
            </p>
            <form className="flex flex-col gap-3" onSubmit={(e) => e.preventDefault()}>
              ...
              />
              <button type="submit" className="btn-primary text-xs justify-center">
                Subscribe
              </button>
            </form>
          </div>
        ...
        <div className="mt-16 pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-ivory/40">
            © {new Date().getFullYear()} Yaw Tutu. All rights reserved.
          </p>
          <ul className="flex items-center gap-6">
            {footerLinks.legal.map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
                  className="text-xs text-ivory/40 hover:text-ivory/70 transition-colors duration-300"
                >
                  {link.label}
                </Link>
              </li>
