  ...
  return (
    <section className="section-padding bg-ivory-200 border-y border-sand/40">
      <div className="container-narrow text-center">
        ...
        <h2 className="heading-lg text-charcoal-800 mb-4 text-balance">The 5 Pillars of Conscious Intimacy</h2>
        <div className="divider-gold mx-auto mb-6" />
        <p className="body-lg text-charcoal-500 mb-10 max-w-xl mx-auto text-pretty">
          A free guide on building deeper connection — covering nervous system awareness, consent as a living practice, and the art of presence.
        </p>
        {status === 'success' ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Your email address" required aria-label="Email address" disabled={status === 'loading'}
            ...
            <button type="submit" disabled={status === 'loading'} className="btn-primary whitespace-nowrap">
              {status === 'loading' ? 'Sending…' : <><span>Send it to me</span> <ArrowRight size={15} /></>}
            </button>
          </form>
        )}
        {status === 'error' && <p className="text-sm text-red-600 mt-3">Something went wrong — try again or email us directly.</p>}
